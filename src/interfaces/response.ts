import type { PIVOT_TIME_FRAME_KEYS } from 'components/StockTechnical/config';
...
import type {
  TIME_STATUS,
  WITHDRAWAL_SUPPORT_READY_STATUS,
} from 'components/WithdrawalSupport/WithdrawalSupportForm/config';
import type {
  GTD_TYPE,
  LOAN_TYPE,
  ORDER_KIND,
...
export interface PreOrderItem {
  ...
  // TODO: fake type avoid ts error, be đang thiếu field
  mthPri: string;
  mthQty: string;
  nmThQty: string;
  bnhCd: string;
...
export interface NotificationItem {
  ...
  content: string;
  icon: string;
  type: 'DEVICE_PUSH' | string;
  category: NOTI_CATEGORY;
  action: 'INFO_ACCOUNT' | null | string;
  scheduledAt: string;
...
export type NotificationParams =
  | CancelOrderNotificationParams
  | MatchOrderNotificationParams
  | Record<string, unknown>
  | {
      monitorPrice: MonitorPrice;
      displayMonitorPrice: DisplayMonitorPrice;
      monitorPriceRuleType: string;
      createdAt: string;
    };

export interface CancelOrderNotificationParams {
  ...
  stk_price: string;
  stk_del_qty: string;
  buy_dt?: string; // Format: "HH:mm:ss"
  sell_dt?: string; // Format: "HH:mm:ss"
}

export interface MatchOrderNotificationParams {
...
export interface AmountInformation {
  ...
  subNo: string; // Số tài khoản phụ
  cash: number; // Tiền mặt/Số dư
  ableDpo: number; // Số tiền có thể đặt lệnh (Able Deposit)
  penWithdrawApr: number; // Số tiền chờ rút về (Pending Withdrawal Approval)
  lndAblAmt: number; // Số tiền có thể vay (Loan Able Amount)
  buyOutBskAmt: number; // Số tiền mua ngoài giỏ (Buy Out Basket Amount)
  blockDpo: number; // Số tiền bị khóa/phong tỏa (Blocked Deposit)
  availableWithdraw: number; // Số tiền có thể rút
  readyStat: WITHDRAWAL_SUPPORT_READY_STATUS; // Trạng thái sẵn sàng
  loanType: string; // Loại khoản vay
}

export interface MaxWithdrawalInfo {
  accountNo: string; // Số tài khoản
  subNo: string; // Số tài khoản phụ
  cashAdv: number; // Ứng tiền/Tiền ứng trước (Cash Advance)
  loanRepay: number; // Số tiền trả nợ vay
  availableWithdraw: number; // Số tiền tối đa có thể rút
  loanType: string; // Loại khoản vay
  status: string; // Trạng thái
  timeStatus: TIME_STATUS; // Trạng thái thời gian (N = Normal, Y = Yes)
}
export interface BannerItem {
  ...
  webTextVi: string | null;
  webTextEn: string | null;
  type: BannerType;
  displayPeriod: number | null;
}

export type BannerAction = 'SHOW_IMAGE' | 'OPEN_LINK' | 'DOWNLOAD_FILE';

export type BannerPlatform = 'MTS' | 'WEB' | 'ALL' | null;

export type BannerType = 'NORMAL' | 'SPECIAL' | 'POPUP';

export interface PredictiveStaticInfo {
  mktId: string;
  stkIsin: string;
  stkId: string;
  stkCd: string;
...
export type NotificationCategory = {
  ...
  requireLogin: number;
  urlLink: string;
  subCategories: NotificationCategory[];
  enable?: boolean;
};

export interface SecuritiesRightsItem {
  stkCode: string;
  exDate: string; //format: yyyymmdd
  message: string;
}
export interface BasicPivotPoints {
...
export interface PivotTimeFrame {
  traditional: BasicPivotPoints;
  fibonacci: BasicPivotPoints;
  camarilla: BasicPivotPoints;
  woodie: BasicPivotPoints;
  demark: BasicPivotPoints;
}

export interface StockPivots {
  [PIVOT_TIME_FRAME_KEYS.ONE_DAY]: PivotTimeFrame; // Daily pivots
  [PIVOT_TIME_FRAME_KEYS.ONE_WEEK]: PivotTimeFrame; // Weekly pivots
  [PIVOT_TIME_FRAME_KEYS.ONE_HOUR]: PivotTimeFrame; // Hourly pivots
}

export interface StockPivotData {
  id: string;
  symbol: string;
  pivots: StockPivots;
}

// Interface cho giá trị của mỗi chỉ báo
export interface RatingValue {
  point_Description: string; // "Bán mạnh" | "Bán" | "Trung lập" | "Mua" | "Mua mạnh"
  signalNote: string; // Ghi chú về tín hiệu
  t0_Value: string; // Giá trị hiện tại (dạng string)
  totalPoint: string; // Tổng điểm (dạng string)
}

// Interface cho mỗi rating item
export interface RatingItem {
  key: string; // VD: "SMA20__15m", "RSI14_Daily", "EMA50_Weekly"
  value: RatingValue;
}

// Interface cho overview rating value
export interface OverviewRatingValue {
  point_Description: string; // "Bán mạnh" | "Bán" | "Trung lập" | "Mua" | "Mua mạnh"
  totalPoint: string; // Tổng điểm phần trăm
}

// Interface cho overview item
export interface OverviewItem {
  key: string; // VD: "Overview_Tong__15m", "Overview_DuongTB_Daily"
  value: OverviewRatingValue;
}

// Interface chính cho Stock TA Rating
export interface StockTARating {
  ratings: RatingItem[]; // Danh sách tất cả các chỉ báo kỹ thuật
  ratingOverviews: OverviewItem[]; // Danh sách tổng quan đánh giá
}

// Interface cho response từ API
export interface StockTARatingResponse {
  id: string;
  symbol: string;
  stockTARating: StockTARating;
}

// Enum cho các timeframe
export enum TA_TIMEFRAME {
  FIFTEEN_MIN = '__15m',
  HOURLY = '_Hourly',
  DAILY = '_Daily',
  WEEKLY = '_Weekly',
...
// Enum cho các loại chỉ báo
export enum TA_INDICATOR_TYPE {
  // Đường trung bình
  SMA20 = 'SMA20',
  SMA50 = 'SMA50',
  SMA200 = 'SMA200',
  ...
  // Bollinger Bands
  BOLLINGER_UPPER = 'BollingerBand_UpperBand',
  BOLLINGER_LOWER = 'BollingerBand_LowerBand',

  // Oscillators
  RSI14 = 'RSI14',
  STOCHASTIC_K = 'Stochastic_SlowK',
  STOCHASTIC_D = 'Stochastic_SlowD',
  CCI = 'CCI',
  MFI = 'MFI',
...
// Enum cho các loại overview
export enum TA_OVERVIEW_TYPE {
  TOTAL = 'Overview_Tong',
  MOVING_AVERAGE = 'Overview_DuongTB',
  TREND = 'Overview_XuHuong',
  MOMENTUM = 'Overview_GiaoDong',
  MONEY_FLOW = 'Overview_DongTien',
}

export interface MonitorPrice {
  minTargetPrice: number;
  buyPrice: number;
  symbol: string;
  isSellTypeSent: boolean;
  isBuyTypeSent: boolean;
  maxTargetPrice: number;
  type: string;
  cutLossPrice: number;
}

export interface DisplayMonitorPrice {
  minTargetPrice: number;
  buyPrice: number;
  symbol: string;
  maxTargetPrice: number;
...
export interface SocketNotification {
  ...
  // backend trả thiếu field method
  method: 'NOTIFICATION_READ';
}
