  ...
  return (
    ...
    >
      ...
      >
        ...
        {props.leadingIcon && (
          <div className={clsx('leading-icon', inEKyc && 'mt-[2rem]')}>{leadingIcon}</div>
        )}
        {props.type === 'textarea' ? (
          <textarea
            {...rest}
        ...
        {clearable && (
          <button
            ...
            tabIndex={-1}
          >
            <XIcon className="size-[2.4rem] text-[--text-2]" />
          </button>
        )}
        {loading && <Preload />}
      </div>
      {props.hasError && !props?.hideErrorMessage && <div className="error-message">{errorMessage}</div>}
    </div>
  );
});
