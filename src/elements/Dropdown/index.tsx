...
const Dropdown = (props: DropdownProps) => {
  const { labelPlacement = 'outside', searchClassName, containerClassName, ...rest } = props;

  const { label, hasError, errorMessage, className, btnClassName, listOptionClassName, hasSearch } = props;
    ...
    >
      ...
      <Listbox disabled={props.disabled} value={selected || ''} onChange={handleChange}>
        <div className={clsx('relative w-full h-full')}>
          <ListboxButton
            className={`dropdown-button shadow-base-1 relative light:bg-white ${btnClassName}`}
          ...
          >
            {labelPlacement === 'outside' && (
              <>
                ...
                >
                  {(selectedOption?.label as string) ?? (props.placeholder as string) ?? ''}
                </span>
              </>
            )}
            {labelPlacement === 'inside' && label != null && (
              <>
                <label className="dropdown-label !mb-[0rem]" htmlFor="" data-label-placement={labelPlacement}>
                  {label}
                </label>
                <span
                  className={clsx(
            ...
            <ChevronDown className="!h-[2.4rem] !w-[2.4rem] !text-[var(--text-5)]" aria-hidden="true" />
          </ListboxButton>
          <Transition
            as={Fragment}
                          ...
                          return (
                            <>
                              {option?.iconUrl && option?.shortName ? (
                                ...
                                <div>
                                  <div
                                    className={clsx('block truncate px-[0.4rem]', props.classNames?.option)}
                                  >
                                    {option.label}
                                  </div>
                                </div>
                    ...
                    <ListboxOption
                      ...
                      className={({ selected }) =>
                        clsx(
                          ...
                          {
                            'border-b border-b-[--border-16] mx-[0.8rem]': idx <= filteredOptions.length - 1,
                          },
                        )
                      }
                      value={option.value}
